...
* **本模組優化**：
  ...
  * **背景 metadata 雜湊檢測**：背景異步掃描 XML 檔案的相對路徑、大小與修改時間，只用 metadata 判斷是否重置 XPath 快取，不再讀取完整 XML 內容，避免大型 Mod 清單啟動時產生額外磁碟 I/O。
...
* **技術細節**：
  ...
  * **背景 metadata 雜湊檢測**：在啟動建構子中利用 `Task.Run` 開啟背景執行緒，只掃描第三方模組與 Config 中 XML 的相對路徑、大小與修改時間。任何 metadata 變更都會清空 XPath 快取；metadata 未變則保留快取，避免讀取完整 XML 內容造成啟動 I/O 回歸。
...
* **貼圖路徑反查最佳化**：貼圖載入後會記錄 `Texture` 到原始路徑的直接對照，圖集與貼圖掃描流程不再需要線性掃描已載入貼圖清單。
