    ...
    public class DelayedActions : MonoBehaviour
    {
        ...
        public void LateUpdate()
        {
            if (XmlChangeDetector.needWriteSettings)
            {
                ...
                {
                    FGLLog.Warning("Failed to save updated XML combined hash: " + ex.Message);
                }
            }
        ...
        {
            stopwatch.Start();
            FGLLog.Message("Starting deferred graphics: " + GraphicsToLoadCount);
            while (GraphicsToLoadCount > 0)
            {
                if (UnityData.IsInMainThread is false)
                {
                    yield return 0;
                }
                while (GraphicsToLoadCount > 0 && !OverBudget)
                {
                    ThingDef def = null;
                    Action action = null;
                    lock (graphicsToLoad)
        ...
        {
            stopwatch.Restart();
            FGLLog.Message("Starting deferred icons: " + IconsToLoadCount);
            while (IconsToLoadCount > 0)
            {
                if (UnityData.IsInMainThread is false)
                {
                    yield return 0;
                }
                while (IconsToLoadCount > 0 && !OverBudget)
                {
        ...
        {
            stopwatch.Restart();
            FGLLog.Message("Starting SubSoundDef resolution: " + SubSoundDefToResolveCount);
            while (SubSoundDefToResolveCount > 0)
            {
                while (SubSoundDefToResolveCount > 0 && !OverBudget)
                {
                    SubSoundDef def = null;
                    Action action = null;
                    lock (subSoundDefToResolve)
                    {
                        if (subSoundDefToResolve.Count > 0)
                ...
                if (SubSoundDefToResolveCount > 0)
                {
                    yield return 0;
                    stopwatch.Restart();
                }
